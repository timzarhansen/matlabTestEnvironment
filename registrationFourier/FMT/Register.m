...
function RegnisterFourierMellin()
    ...
    I1 = imread('lena_cropped.bmp');
    %I1 = imread('firstImage.jpg');
    
    

    % Load second image (I2)

    I2 = imread('lena_cropped_rotated.bmp');
    %I2 = imread('secondImage.jpg');
...
%     
    ...
    figure(3)
    imagesc(abs(IA))
    title('Magnitude HP 1')
    figure(4)
    imagesc(abs(IB))
    title('Magnitude HP 2')
    ...
    figure(7)
    [Xplot,Yplot]=meshgrid(1:size(THETA_PHASE,1),1:size(THETA_PHASE,1));
    surf(Xplot,Yplot,((THETA_PHASE)));
    xlabel("x-axis");
    ylabel("y-axis");
    title('result of Rotation')
