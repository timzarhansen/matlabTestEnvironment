...
for j = 1:numberOfSolutions
    ...
    tform = eye(4);
    tform(1:3,1:3) = rotationMatrix(0,0,-0.1);%pi/2+0.1
    tform(1,4)=0;%x value
    tform(2,4)=0;%y value
...
%     ylabel('Y-Axis in m', 'Interpreter', 'latex')
    title(num2str(j), 'Interpreter', 'latex','Color', 'k')
end
...
if 1
    ...
    tform = eye(4);
    tform(1:3,1:3) = rotationMatrix(0,0,1.6);
    tform(1,4)=0;%x value
    tform(2,4)=0;%y value
    tform=affine3d(tform');
    pointCloudResult1 = pctransform(pointCloudResult1,tform);
